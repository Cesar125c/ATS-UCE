...
def setup_logging():
    ...
    logging_config = {
        ...
        "formatters": {
            "standard": {
                "()": ISO8601Formatter,
                "format": "%(asctime)s | %(levelname)-5s | [%(request_id)s] | [%(usuario_id)s] | [%(evento)s] | [%(endpoint)s] | %(message)s | %(duracion_ms)s",
            }
        },
        "handlers": {
            "console": {
        ...
        "loggers": {
            ...
            # Uvicorn access logs (muted to WARNING, to prepare for our custom HTTP logging middleware)
            "uvicorn.access": {
                "level": "WARNING",
                "handlers": ["console"],
                "propagate": False,

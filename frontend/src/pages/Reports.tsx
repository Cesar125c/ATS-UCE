...
export default function Reports() {
  ...
  const pieData = stats
    ? [
        ...
        { name: "Rejected", value: Math.max(0, stats.total_applicants - stats.in_progress - stats.completed) },
      ]
    : [];
    ...
    <DashboardLayout>
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-2">
          Statistics and metrics of the faculty selection process.
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-8">
        {[
          { label: "Total Applicants", value: stats?.total_applicants ?? 0, icon: Users, color: "bg-blue-100 text-blue-600" },
          { label: "Average Score", value: `${Math.round(stats?.avg_score ?? 0)}%`, icon: TrendingUp, color: "bg-green-100 text-green-600" },
          { label: "In Progress", value: stats?.in_progress ?? 0, icon: Clock3, color: "bg-orange-100 text-orange-600" },
          { label: "Selected", value: stats?.completed ?? 0, icon: CheckCircle2, color: "bg-purple-100 text-purple-600" },
        ].map((card) => {
